...
@Client.on_message(MessageHandler(filters=pyrogram.Filters.command(["start"])))
async def start(c, m):
...
@Client.on_message(MessageHandler(filters=pyrogram.Filters.command(["help"])))
async def help(c, m):
    await c.send_message(chat_id=m.chat.id,
...
@Client.on_message(MessageHandler(filters=pyrogram.Filters.command(["about"])))
async def about(c, m):
    await c.send_message(chat_id=m.chat.id,
...
@Client.on_message(MessageHandler(filters=pyrogram.Filters.command(["converttovideo"])))
async def video(c, m):
    if Config.BOT_PWD:
...
@Client.on_message(MessageHandler(filters=pyrogram.Filters.command(["converttofile"])))
async def file(c, m):
    if Config.BOT_PWD:
        if (m.from_user.id not in Config.LOGGED_USER) & (m.from_user.id not in Config.AUTH_USERS):
            await m.reply_text(text=Translation.NOT_LOGGED_TEXT, quote=True)
...
@Client.on_message(MessageHandler(filters=pyrogram.Filters.command(["login"])))
async def login(c, m):
    if Config.BOT_PWD:
